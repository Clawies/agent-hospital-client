...
export function generateEd25519Keypair(): {
  rawPublicKey: Buffer;
  publicKeyBase64: string;
  privateKeyJwk: StoredCredentials["privateKeyJwk"];
} {
  ...
  const jwk = kp.privateKey.export({ format: "jwk" }) as {
    kty: string;
    crv: string;
    x: string;
    d: string;
...
export function computeFingerprint(rawPublicKey: Buffer): string {
...
export function loadCredentials(hospitalUrl: string): StoredCredentials | null {
  try {
    const raw = fs.readFileSync(CRED_FILE, "utf-8");
    const creds: StoredCredentials = JSON.parse(raw);
    if (creds.hospitalUrl !== hospitalUrl) return null;
    if (!creds.fingerprint || !creds.privateKeyJwk?.d) return null;
    return creds;
  } catch {
    return null;
  }
...
export async function registerAndCache(
  ...
  const enrollmentToken: string = hostResp.enrollmentToken;
  if (!enrollmentToken) {
    throw new Error("Host registration did not return an enrollment token");
  }
  if (!json) console.error(`  Host registered.`);

  // Step 2: Generate Ed25519 keypair
  const { rawPublicKey, publicKeyBase64, privateKeyJwk } =
  ...
  const agentId: string = agentResp.agentId;
  if (!agentId) {
    throw new Error("Agent registration did not return an agent ID");
  }
  if (!json) console.error(`  Agent registered (${agentId.slice(0, 8)}...).`);

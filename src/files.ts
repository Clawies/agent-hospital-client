...
function collectPartialDir(
  ...
  const fileInfos: { name: string; abs: string; rel: string; size: number; mtimeMs: number }[] = [];
  ...
  if (isLogDir) {
    ...
    for (const nf of newest) {
      if (currentBytes.value >= MAX_TOTAL_BYTES) break;
      try {
...
function incrementSkip(stats: ScanStats, reason: string): void {
...
export function collectFileContents(framework: Framework): Record<string, string> {
  ...
  const scanRoots: { path: string; label: string }[] = [];
  ...
  for (const entry of fileEntries) {
    ...
    try {
      ...
      if (isBinary(buf)) {
        ...
        if (treeIdx >= 0) tree[treeIdx].skipReason = "binary-content";
        continue;
      }

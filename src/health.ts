...
// ---------------------------------------------------------------------------
// Types -- matches the server's health-protocol.ts HealthReport schema exactly
// ---------------------------------------------------------------------------
...
export interface LogAnalysis {
  ...
  topErrors: Array<{ pattern: string; count: number; lastSeen: string }>;
  errorRate: string;
}
...
export interface WorkspaceHealth {
  files: Record<string, { exists: boolean; wordCount: number; lineCount: number; sizeBytes: number; lastModified: string | null }>;
  skills: { count: number; names: string[]; validation: SkillValidation[] };
  pipeline: { exists: boolean; stageCount: number };
}
...
function getSafeConfigSnapshot(config: any): Record<string, unknown> {
  const snapshot = JSON.parse(JSON.stringify(config || {}));
  const redact = (obj: any, depth = 0): void => {
...
function httpGet(port: number, urlPath: string, timeoutMs = 5000): Promise<{ statusCode: number; body: string; latencyMs: number }> {
...
async function probeGateway(port: number): Promise<ProbeResult> {
  const start = Date.now();
  try {
...
async function probeModelApi(config: any): Promise<ProbeResult> {
  const start = Date.now();
  ...
  try {
    if (provider === "anthropic") {
      ...
      const result = await new Promise<{ statusCode: number; body: string; latencyMs: number }>((resolve, reject) => {
...
function probeProcessHealth(pid: number | null): ProbeResult {
  const start = Date.now();
...
function probeSqliteIntegrity(framework: Framework): ProbeResult {
  const start = Date.now();
  ...
  for (const dbPath of dbPaths) {
    ...
    try {
      const output = execSync(
        `sqlite3 "${dbPath}" "PRAGMA integrity_check; PRAGMA page_count; PRAGMA freelist_count;"`,
...
function probeCronValidity(crons: Array<{ id: string; expression: string; nextRun: string | null }>): ProbeResult {
  const start = Date.now();
...
function probeErrorCount(framework: Framework): ProbeResult {
  const start = Date.now();
  const homeDir = os.homedir();
  ...
  try {
    ...
    const files = fs.readdirSync(logsDir)
      ...
      .map((f) => {
        try { return { name: f, mtime: fs.statSync(path.join(logsDir, f)).mtime }; } catch { return null; }
      })
...
async function probeIntegrationConnectivity(config: any): Promise<ProbeResult[]> {
...
async function runAllProbes(
  ...
  pid: number | null,
  config: any,
  crons: Array<{ id: string; expression: string; nextRun: string | null }>
): Promise<ProbeResult[]> {
  const results: ProbeResult[] = [];
...
function findOpenClawWorkspaces(): string[] {
...
function findOpenClawPid(): number | null {
...
function getOpenClawPersonality(workspaces: string[]): PersonalityHealth {
...
function getOpenClawWorkspaceFileInfo(workspaces: string[], fileName: string): { exists: boolean; wordCount: number; lineCount: number; sizeBytes: number; lastModified: string | null } {
  for (const ws of workspaces) {
...
function getOpenClawMemory(workspaces: string[]): MemoryHealth {
  let sizeBytes = 0;
  let entryCount = 0;
    ...
    try {
      ...
      for (const e of entries) {
        try {
          ...
          if (!lastWrite || mtime > lastWrite) lastWrite = mtime;
        } catch {}
      }
    } catch {}
  }
...
function getOpenClawIntegrations(config: any): IntegrationHealth[] {
  const integrations: IntegrationHealth[] = [];
  const channels = config?.channels || {};
  for (const [platform, settings] of Object.entries(channels)) {
    integrations.push({
      type: platform,
      connected: !!(settings as any)?.enabled,
      tokenExpiresAt: null,
      lastMessageAt: null,
      error: null,
    });
...
function getOpenClawCrons(): CronJobHealth[] {
  ...
  try {
    ...
    for (const j of jobs) {
      crons.push({
        ...
        lastStatus: (j.lastStatus as "success" | "error" | "unknown") || "unknown",
...
function getOpenClawModel(config: any): ModelHealth {
  const envConfig = config?.env || {};
...
function getOpenClawDisk(workspaces: string[]): DiskHealth {
  ...
  for (const ws of workspaces) {
    try {
      ...
      for (const e of entries) {
        if (e.endsWith(".jsonl")) {
          try { sessionsBytes += fs.statSync(path.join(ws, e)).size; } catch {}
        }
      }
    } catch {}
  }
...
function getOpenClawLogsTail(): string | null {
...
function getOpenClawSkillValidation(workspaces: string[], soulContent: string | null): SkillValidation[] {
...
function getOpenClawWorkspaceDetails(workspaces: string[]): WorkspaceHealth {
  ...
  const soulContent = files["soul"]?.exists ? (() => {
    for (const ws of workspaces) {
      try { return fs.readFileSync(path.join(ws, "SOUL.md"), "utf-8"); } catch {}
    }
    return null;
  })() : null;

  const skillValidation = getOpenClawSkillValidation(workspaces, soulContent);
  let skills = { count: 0, names: [] as string[], validation: skillValidation };
  for (const ws of workspaces) {
    const skillsDir = path.join(ws, "skills");
    try {
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
      const names = entries
        .filter((e: any) => e.isDirectory() || e.name.endsWith(".md") || e.name.endsWith(".json"))
        .map((e: any) => e.name);
...
function getOpenClawSessionHealth(workspaces: string[]): SessionHealth {
  let totalCount = 0;
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        const mtime = stat.mtime.toISOString();
        if (!oldestTimestamp || mtime < oldestTimestamp) oldestTimestamp = mtime;
        if (!newestTimestamp || mtime > newestTimestamp) newestTimestamp = mtime;
        if (stat.mtime.getTime() < thirtyDaysAgo) olderThan30Days++;
      } catch {}
    }
  } catch {}
...
function getOpenClawLogAnalysis(): LogAnalysis {
  const logsDir = path.join(OPENCLAW_DIR, "logs");
  let totalErrors = 0;
  let totalWarnings = 0;
...
async function collectOpenClawHealth(): Promise<HealthReport> {
  ...
  const probes = await runAllProbes(
    "openclaw",
    OPENCLAW_DAEMON_PORT,
    pid,
    config,
    crons.map((c) => ({ id: c.id, expression: c.expression, nextRun: c.nextRun }))
  );
...
function getHermesPersonality(): PersonalityHealth {
  const candidates = [
...
function getHermesMemory(): MemoryHealth {
  let sizeBytes = 0;
  let entryCount = 0;
  let storeReachable = false;
  let lastWrite: string | null = null;
  ...
  return { type: memType as MemoryHealth["type"], entryCount, storeReachable, lastWrite, sizeBytes };
}

function getHermesIntegrations(config: any): IntegrationHealth[] {
  const integrations: IntegrationHealth[] = [];
...
function getHermesCrons(config: any): CronJobHealth[] {
  const crons: CronJobHealth[] = [];
...
function getHermesModel(config: any): ModelHealth {
...
function getHermesDisk(): DiskHealth {
...
function getHermesLogsTail(): string | null {
...
function getHermesVersion(config: any): string | null {
...
function getHermesSessionHealth(): SessionHealth {
  let totalCount = 0;
  let oldestTimestamp: string | null = null;
  let newestTimestamp: string | null = null;
  let olderThan30Days = 0;
...
function getHermesLogAnalysis(): LogAnalysis {
  let totalErrors = 0;
  let totalWarnings = 0;
  const errorCounts = new Map<string, { count: number; lastSeen: string }>();
...
function getHermesSkillValidation(soulContent: string | null): SkillValidation[] {
  const validations: SkillValidation[] = [];
  const soulLower = (soulContent || "").toLowerCase();
...
function getHermesWorkspaceDetails(): WorkspaceHealth {
  const files: Record<string, any> = {};
  ...
  const soulContent = files["soul"]?.exists ? (() => {
    try { return fs.readFileSync(path.join(HERMES_DIR, "SOUL.md"), "utf-8"); } catch { return null; }
  })() : null;
...
async function collectHermesHealth(): Promise<HealthReport> {
...
export async function collectHealth(framework: Framework): Promise<HealthReport> {
  try {
    if (framework === "hermes") {

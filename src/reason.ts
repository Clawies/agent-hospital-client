...
// ---------------------------------------------------------------------------
// Agent reasoning -- the patient thinks about the doctor's diagnosis
// ---------------------------------------------------------------------------
...
export async function reasonAboutDiagnosis(
  framework: Framework,
  narrative: string,
  commands: DiagnosisFinding[],
...
function buildSelfKnowledge(framework: Framework, soulContent: string | null): string {
  const parts: string[] = [];
...
Respond with valid JSON only:
{
  ...
  const commandList = commands.map((c, i) =>
    `${i + 1}. [${c.whitelisted ? "AUTO" : "MANUAL"}] ${c.action}\n   ${c.description}`
  ).join("\n");
...
async function callLocalLLM(proxyUrl: string, prompt: { system: string; user: string }): Promise<string> {
...
function parseAgentFeedback(response: string, commands: DiagnosisFinding[]): AgentFeedback {
  // Strip code fences
  ...
  const disagreements: Array<{ action: string; reason: string }> = [];
  const executeOnly: string[] = [];
